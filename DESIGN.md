...
- **Data pipeline:** `data/languages/*.json` (edited by hand/PR) → `deno task validate` → `deno task build:db` →
  ...
  <!--languages-->72<!--/--> languages ≈ a few hundred KB, so everything fits in memory.
...
| **Task / Snippet**   | <!--tasks-->6<!--/--> shared tasks (hello, factorial, fizzbuzz, map&filter, shapes, signature) × <!--languages-->72<!--/--> languages, each with `expected_output` and `verified`; signature snippets tag the concepts they demonstrate (`concepts`) |
...
- **Trait matrix:** <!--languages-->72<!--/--> languages × <!--concepts-->56<!--/--> concepts, as a heat grid. Sort by
...
1. **Data** ✅ <!--languages-->72<!--/--> languages, <!--concepts-->56<!--/--> concepts, <!--edges-->346<!--/--> lineage
   edges (<!--cross-pollination-->24<!--/--> "cross-pollination": a newer language feeding a later version of an older
   one, e.g. Kotlin → PHP 8, drawn separately), <!--snippets-->432<!--/--> snippets (AppleScript's stay unverified: it
   exists only on macOS). <!--images-->40<!--/--> custom images in `sandbox/<id>/Dockerfile` (ALGOL 60 via MARST, Simula
...
4. **Runner** ✅ local runner with SSE streaming. `deno task verify` passes
   <!--verified-->426<!--/-->/<!--runnable-->426<!--/--> runnable snippets on real toolchains. CI verifies the languages
...
5. **Genealogy + Genome** ✅ `/tree` has three views. The family tree is a time-scaled dendrogram by primary parent. Its
   ...
   <!--forward-edges-->322<!--/--> non-retro edges top to bottom, using ELK `layered` at build time
...
6. **Concept pages** ✅ `/concepts` lists all <!--concepts-->56<!--/--> by category with an adoption sparkline.
   ...
   the languages without it, and the concepts most often found with it (Jaccard over carriers). <!--tagged-->59<!--/-->
...
- Hosting: **decided: read-only on Cloudflare Workers** (`deno task deploy`); code windows show recorded output and
  ...
  abuse controls (§4).
- Scope growth: <!--languages-->72<!--/--> → 100+? Added so far: AWK, Object Pascal, Oberon, J, Racket, Nim, Dart,
  Gleam, HyperTalk, AppleScript, NewtonScript, Dylan, Mojo, Logo, Bourne shell, Lean, Odin, Unison, Elm, Crystal, Visual
  Basic (run as VB.NET), QBasic. **Next up**, in order: MATLAB (via GNU Octave), D, PowerShell, Koka, Luau, Roc, Bend.
