...
- **Data pipeline:** `data/languages/*.json` (edited by hand/PR) → `deno task validate` → `deno task build:db` →
  `dist/`. The app loads `world.json` at startup; SQLite is for search and exploration queries.
  <!--languages-->68<!--/--> languages ≈ a few hundred KB, so everything fits in memory.
...
| **Task / Snippet**   | <!--tasks-->6<!--/--> shared tasks (hello, factorial, fizzbuzz, map&filter, shapes, signature) × <!--languages-->68<!--/--> languages, each with `expected_output` and `verified`; signature snippets tag the concepts they demonstrate (`concepts`) |
...
- **Trait matrix:** <!--languages-->68<!--/--> languages × <!--concepts-->56<!--/--> concepts, as a heat grid. Sort by
...
1. **Data** ✅ <!--languages-->68<!--/--> languages, <!--concepts-->56<!--/--> concepts, <!--edges-->331<!--/--> lineage
   edges (<!--cross-pollination-->23<!--/--> "cross-pollination": a newer language feeding a later version of an older
   one, e.g. Kotlin → PHP 8, drawn separately), <!--snippets-->408<!--/--> snippets (AppleScript's stay unverified: it
   exists only on macOS). <!--images-->37<!--/--> custom images in `sandbox/<id>/Dockerfile` (ALGOL 60 via MARST, Simula
   via Cim, LISP 1.5, BCPL, B, CLU, Self, Miranda, HyperTalk via LiveCode Server, Logo via a headless UCBLogo…). Static
   UI prototype: `design/prototype/` (`deno task prototype`).
...
3. **Rosetta Desk** ✅ period-skinned windows, a CodeMirror 6 editor (`web/lib/editor.ts`), a language switcher, Run all
   ...
   runs.
4. **Runner** ✅ local runner with SSE streaming. `deno task verify` passes
   <!--verified-->402<!--/-->/<!--runnable-->402<!--/--> runnable snippets on real toolchains. CI verifies the languages
...
5. **Genealogy + Genome** ✅ `/tree` has three views. The family tree is a time-scaled dendrogram by primary parent. Its
   rows are an in-order walk, so no connector crosses a label. The influence web lays out all
   <!--forward-edges-->308<!--/--> non-retro edges top to bottom, using ELK `layered` at build time
...
6. **Concept pages** ✅ `/concepts` lists all <!--concepts-->56<!--/--> by category with an adoption sparkline.
   ...
   the languages without it, and the concepts most often found with it (Jaccard over carriers). <!--tagged-->55<!--/-->
...
- Hosting: **decided: read-only on Cloudflare Workers** (`deno task deploy`); code windows show recorded output and
  ...
  abuse controls (§4).
- Scope growth: <!--languages-->68<!--/--> → 100+? Added so far: AWK, Object Pascal, Oberon, J, Racket, Nim, Dart,
  Gleam, HyperTalk, AppleScript, NewtonScript, Dylan, Mojo, Logo, Bourne shell, Lean, Odin, Unison. **Next up**, in
  order: Elm (already cited by Gleam, Rust and Unison), MATLAB (via GNU Octave), Visual Basic (VB.NET), D, PowerShell,
  Crystal, Koka, Luau, Roc, Bend. Later: Idris 2, Pony, Nushell, GDScript, Carbon, WebAssembly text, Raku, Hare;
  historic gaps the data cites most: Modula-3, Mesa, ISWIM, Newsqueak, PL/I, SNOBOL. See "Adding a language" in
  `data/README.md`.
