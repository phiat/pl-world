...
- **Data pipeline:** `data/languages/*.json` (edited by hand/PR) → `deno task validate` → `deno task build:db` →
  ...
  queries. <!--languages-->80<!--/--> languages ≈ a few hundred KB, so everything fits in memory.
...
| **Task / Snippet**   | <!--tasks-->6<!--/--> shared tasks (hello, factorial, fizzbuzz, map&filter, shapes, signature) × <!--languages-->80<!--/--> languages, each with `expected_output` and `verified`; signature snippets tag the concepts they demonstrate (`concepts`) |
...
- **Trait matrix:** <!--languages-->80<!--/--> languages × <!--concepts-->56<!--/--> concepts, as a heat grid. Sort by
...
1. **Data** ✅ <!--languages-->80<!--/--> languages, <!--concepts-->56<!--/--> concepts, <!--edges-->391<!--/--> lineage
   edges (<!--cross-pollination-->26<!--/--> "cross-pollination": a newer language feeding a later version of an older
   one, e.g. Kotlin → PHP 8, drawn separately), <!--snippets-->480<!--/--> snippets (AppleScript's stay unverified: it
   exists only on macOS). <!--images-->47<!--/--> custom images in `sandbox/<id>/Dockerfile` (ALGOL 60 via MARST, Simula
...
4. **Runner** ✅ local runner with SSE streaming. `deno task verify`
   passes <!--verified-->474<!--/-->/<!--runnable-->474<!--/--> runnable snippets on real toolchains. CI verifies the languages
...
5. **Genealogy + Genome** ✅ `/tree` has three views. The family tree is a time-scaled dendrogram by primary parent. Its
   ...
   all <!--forward-edges-->365<!--/--> non-retro edges top to bottom, using ELK `layered` at build time
...
6. **Concept pages** ✅ `/concepts` lists all <!--concepts-->56<!--/--> by category with an adoption sparkline.
   ...
   the languages without it, and the concepts most often found with it (Jaccard over carriers). <!--tagged-->67<!--/-->
...
- Hosting: **decided: read-only on Cloudflare Workers** (`deno task deploy`); code windows show recorded output and
  ...
  abuse controls (§4).
- Scope growth: <!--languages-->80<!--/--> → 100+? Added so far: AWK, Object Pascal, Oberon, J, Racket, Nim, Dart,
  Gleam, HyperTalk, AppleScript, NewtonScript, Dylan, Mojo, Logo, Bourne shell, Lean, Odin, Unison, Elm, Crystal, Visual Basic (run as VB.NET), QBasic, MATLAB (run on GNU Octave), D, PowerShell, V, Modula-3, Icon, SNOBOL (run as SNOBOL4), Tcl.
  **Next up**, in order: Koka, Luau, Roc, Bend.
  ...
  Mesa, ISWIM, Newsqueak, PL/I. See "Adding a language" in `data/README.md`.
